...
To start with, I needed to find an ICE.  I went to Microsoft's Feedback Reporting Tool and searched through until I found one where it was not entirely obvious what the bug was.  ultimately I chose [this one](https://developercommunity.visualstudio.com/content/problem/478558/internal-compiler-error-23.html).  The code given by the bug author is pretty small, so I can just paste it here:
...
void bar()
{
	[]()
	{
		constexpr bool baz = std::is_same_v<void, Type>;
...
but can we make it smaller?  This code includes a massive header file with complex template metaprogramming, potentially masking the underlying cause of the ICE.  Is it related to the type trait?  The `if constexpr`?  The lambda?  None of the above?

To start with, let's preprocess the source file

```
; So you know what version I tested this with.
$ cl /?
Microsoft (R) C/C++ Optimizing Compiler Version 19.20.27508.1 for x64
Copyright (C) Microsoft Corporation.  All rights reserved.

; Pre-process to eliminate dependencies on header files so that creduce has a single self-contained source file to work with.
$ cl /c /std:c++17 /P /EP /Fiice2.cpp ice.cpp

; Make sure the problem still reproduces
$ cl /c /std:c++17 ice2.cpp
Microsoft (R) C/C++ Optimizing Compiler Version 19.20.27508.1 for x64
Copyright (C) Microsoft Corporation.  All rights reserved.

ice2.cpp
ice2.cpp(13705): fatal error C1001: An internal error has occurred in the compiler.
(compiler file 'd:\agent\_work\1\s\src\vctools\Compiler\CxxFE\sl\p1\c\ParseTree.cpp', line 3869)
 To work around this problem, try simplifying or changing the program near the locations listed above.
Please choose the Technical Support command on the Visual C++
 Help menu, or open the Technical Support help file for more information
ice2.cpp(13719): note: see reference to function template instantiation 'void bar<void>(void)' being compiled

; Now run creduce!
$ python reduce.py --source=ice2.cpp --creduce=C:\src\creduce-build\creduce\creduce --stdout="fatal error C1001" --stderr="fatal error C1001" --cflags="/c /std:c++17"
```

After a while, we're left with a mere 27 bytes!  
...
This is a much better bug report!  But isn't it weird that it doesn't use `constexpr`, `type_traits`, or really anything else for that matter?  Is this really an ICE?  Let's see:
...
ice2.cpp
ice2.cpp(2): fatal error C1001: An internal error has occurred in the compiler.
(compiler file 'msc1.cpp', line 1527)
 To work around this problem, try simplifying or changing the program near the locations listed above.
Please choose the Technical Support command on the Visual C++
 Help menu, or open the Technical Support help file for more information
Internal Compiler Error in C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\VC\Tools\MSVC\14.20.27508\bin\HostX64\x64\cl.exe.  You will be prompted to send an error report to Microsoft later.
INTERNAL COMPILER ERROR in 'C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\VC\Tools\MSVC\14.20.27508\bin\HostX64\x64\cl.exe'
    Please choose the Technical Support command on the Visual C++
    Help menu, or open the Technical Support help file for more information
```

It's definitely an ICE, but if you look carefully, it appears to be a *different* ICE.  The first one failed in the file `compiler file 'd:\agent\_work\1\s\src\vctools\Compiler\CxxFE\sl\p1\c\ParseTree.cpp', line 3869` and the second one in `compiler file 'msc1.cpp', line 1527`.

Remember, all our interestingness test checked for was some kind of ICE.  So every step of the way, creduce made smaller and smaller programs that ICEd, but we didn't ensure we were getting the same ICE every time.  Let's try again:

This time we'll add the exact failing source file location in the compiler, so as to require that it's the exact same ICE.

```
$ python reduce.py --source=ice2.cpp --cores=12 --creduce=C:\src\creduce-build\creduce\creduce --stdout="d:\agent\_work\1\s\src\vctools\Compiler\CxxFE\sl\p1\c\ParseTree.cpp', line 3869" --stderr="d:\agent\_work\1\s\src\vctools\Compiler\CxxFE\sl\p1\c\ParseTree.cpp', line 3869" --cflags="/c /std:c++17"
```
